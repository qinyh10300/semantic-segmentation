...
def add_multiple_patches_to_background(background_path, img_folder, num_patches=5):
    ...
    # 保存平滑处理后的图像
    output_path = "sample_generation/output_with_multiple_patches_smoothed.bmp"
    cv2.imwrite(output_path, background)
    print(f"Smoothed image with multiple patches saved as {output_path}")

# 使用示例
background_path = "sample_generation/Background.bmp"  # 替换为背景图片路径
img_folder = "sample_generation/qipao_matched"  # 替换为包含小图片的文件夹路径
